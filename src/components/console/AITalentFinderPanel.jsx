...
import { Sparkles, User, RefreshCw, ChevronDown, ChevronUp, Send, Maximize2, Minimize2, X, Plus } from "lucide-react";
...
export default function AITalentFinderPanel() {
  ...
  const toggleManualFilter = (sectionKey, option, single) => {
    setManualFilters((prev) => {
      ...
      return { ...prev, [sectionKey]: [...current, option] };
    });
  };
  ...
  return (
    <div className={`bg-white mt-5 mr-2 ml-2 rounded-3xl border-r border-gray-200 flex flex-col overflow-hidden transition-all duration-700 ${isMaximized ? "w-[700px]" : "w-[400px]"}`} style={{ height: "calc(100vh - 80px)" }}>
      {/* Top Tabs */}
      <div className="px-4 py-4">
        <div className="mb-4 flex items-center">
          <div className="bg-slate-100 px-1 py-1 w-full rounded-[10px] flex gap-1">
            <button
              onClick={() => setActiveMode("ai")} className={`flex-1 px-4 py-2 text-xs font-medium rounded-lg transition-colors ${activeMode === "ai" ? "bg-white text-gray-900 shadow-sm" : "text-gray-400 hover:text-gray-600"}`}>


              Explore with Ai
            </button>
            <button
              onClick={() => setActiveMode("manual")} className={`flex-1 px-4 py-2 text-xs font-medium rounded-lg transition-colors ${activeMode === "manual" ? "bg-white text-gray-900 shadow-sm" : "text-gray-400 hover:text-gray-600"}`}>
      ...
      {<div className="p-4 bg-white">
        <div className="flex gap-2 items-center">
          <input
            ...
            placeholder="Let SproutsAI assist you" className="bg-gray-50 text-[13px] px-4 py-2.5 rounded-[28px] flex-1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400" />
