      ...
      <div className="flex gap-2 flex-wrap mb-4">
        {filterOptions.map((opt) => (
          <button
            ...
            } ${opt.key === "pending" && filter !== "pending" && pendingCount > 0 ? "border-orange-300 text-orange-600 bg-orange-50" : ""}`}>
            {opt.label}
            {opt.key === "pending" && pendingCount > 0 && (
              <span className="ml-1.5 bg-orange-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">{pendingCount}</span>
            )}
          </button>
        ))}
      </div>
