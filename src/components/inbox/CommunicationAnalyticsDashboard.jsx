          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {actionRequired.map((item, idx) => (
                <div
                  key={idx}
                  className={`p-4 rounded-lg border-2 ${item.color} cursor-pointer hover:shadow-sm transition-all`}
                >
                  <p className="text-[11px] font-medium mb-2">{item.label}</p>
                  <p className="text-[24px] font-bold">{item.count}</p>
                </div>
              ))}
            </div>
          </div>
