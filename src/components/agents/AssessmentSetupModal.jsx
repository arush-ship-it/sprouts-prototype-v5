...
import { X, Sparkles, FileText, PenLine, ChevronRight, ChevronLeft, Check, Mail, Clock, Shield, Bot, AlertTriangle, Eye, Loader2, Plus, Trash2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Switch } from "@/components/ui/switch";
import { Textarea } from "@/components/ui/textarea";

const STEPS = [
  { id: 1, label: "Creation Method" },
  { id: 2, label: "Assessment Setup" },
  { id: 3, label: "Review Questions" },
  { id: 4, label: "Invitation Email" },
  { id: 5, label: "Reminder" },
  { id: 6, label: "Filtering & Approval" },
];
      ...
      {STEPS.map((step, idx) => (
        <React.Fragment key={step.id}>
          <div className="flex flex-col items-center gap-1.5">
            ...
            <span className={`text-[10px] font-medium whitespace-nowrap ${currentStep === step.id ? "text-indigo-600" : "text-gray-400"}`}>
              {step.label}
            </span>
          </div>
          {idx < STEPS.length - 1 && (
            <div className={`flex-1 h-[2px] mx-2 mb-3 rounded-full transition-all ${currentStep > step.id ? "bg-indigo-400" : "bg-gray-200"}`} />
          )}
        </React.Fragment>
      ))}
    </div>
...
function Step1({ value, onChange }) {
  const options = [
    {
      ...
      desc: "Let AI create a tailored assessment based on the job role and requirements.",
      badge: "Recommended",
    ...
    },
    {
      ...
      desc: "Start from a pre-built assessment template for common roles.",
      badge: "Quick",
    ...
    },
    {
      ...
      desc: "Build your own assessment entirely from scratch with full control.",
      badge: "Custom",
  ...
  return (
    <div className="space-y-4">
      <div className="mb-6">
        <h2 className="text-[18px] font-bold text-gray-900 mb-1">How would you like to create this assessment?</h2>
        <p className="text-[13px] text-gray-500">Choose the method that works best for you</p>
      </div>
        ...
        {options.map((opt) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-start gap-4 px-5 py-5 rounded-2xl border-2 text-left transition-all ${
                selected ? "border-indigo-400 bg-indigo-50/60" : "border-gray-100 bg-white hover:border-indigo-200 hover:bg-indigo-50/20"
              }`}
            >
...
function Step2({ config, onChange }) {
  return (
    <div className="space-y-6">
      <div className="mb-2">
        <h2 className="text-[18px] font-bold text-gray-900 mb-1">Configure your assessment</h2>
        <p className="text-[13px] text-gray-500">Set the difficulty and question mix</p>
      </div>

      {/* Difficulty */}
      <div>
        <p className="text-[12px] font-semibold text-gray-700 mb-3 uppercase tracking-wide">Difficulty Level</p>
      ...
      {/* Question Mix */}
      <div>
        <p className="text-[12px] font-semibold text-gray-700 mb-3 uppercase tracking-wide">Question Mix</p>
        <div className="space-y-4">
          {[
            { key: "coding", label: "Coding Questions", desc: "Algorithmic & implementation problems", color: "text-blue-600", bg: "bg-blue-50" },
            { key: "mcq", label: "Multiple Choice (MCQ)", desc: "Conceptual and knowledge-based questions", color: "text-emerald-600", bg: "bg-emerald-50" },
            ...
            <div key={key} className={`${bg} rounded-2xl px-5 py-4 flex items-center gap-4`}>
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={() => onChange({ ...config, [key]: Math.max(0, (config[key] || 0) - 1) })}
                ...
                <button
                  onClick={() => onChange({ ...config, [key]: (config[key] || 0) + 1 })}
                  className="w-7 h-7 rounded-lg bg-white border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-gray-50 font-bold"
                >+</button>
              </div>
            </div>
        ...
        <div className="mt-3 flex items-center justify-between px-1">
          ...
          <p className="text-[14px] font-bold text-gray-900">
            {(config.coding || 0) + (config.mcq || 0) + (config.descriptive || 0)}
          </p>
        </div>
      </div>
      ...
      {/* Duration */}
      <div>
        <p className="text-[12px] font-semibold text-gray-700 mb-3 uppercase tracking-wide">Duration</p>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-[18px] font-bold text-gray-900 mb-0.5">Review your questions</h2>
          <p className="text-[13px] text-gray-500">Edit, reorder, or remove any questions before saving</p>
        </div>
        <button className="flex items-center gap-1.5 text-[12px] text-indigo-600 font-medium hover:text-indigo-700 border border-indigo-200 px-3 py-1.5 rounded-lg hover:bg-indigo-50 transition-colors">
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h2 className="text-[18px] font-bold text-gray-900 mb-0.5">Assessment Invitation Email</h2>
          <p className="text-[12px] text-gray-500">Customize the email candidates receive</p>
        </div>
      </div>
      <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-4 flex items-start gap-2">
        <AlertTriangle className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
        <p className="text-[11px] text-amber-700 leading-relaxed">
          Variables in <span className="font-mono font-bold">{`{{double_braces}}`}</span> are auto-filled by the system — candidate name, job title, assessment link, etc.
        </p>
      </div>
      <div className="space-y-3">
        <div>
          <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Subject Line</label>
          <input
        ...
        <div>
          <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Email Body</label>
          <Textarea
            value={emailContent}
            onChange={(e) => onChange(e.target.value)}
            className="w-full min-h-[260px] border border-gray-200 rounded-xl px-3 py-2.5 text-[13px] text-gray-800 font-mono leading-relaxed focus:outline-none focus:ring-1 focus:ring-indigo-400 resize-none bg-white"
          />
        </div>
      </div>
    </div>
    ...
    <div>
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-[12px] text-gray-500">Automatically nudge candidates before the deadline</p>
        </div>
      </div>

      <div className={`border-2 rounded-2xl px-5 py-4 mb-4 flex items-center justify-between transition-all ${reminder.enabled ? "border-amber-300 bg-amber-50/50" : "border-gray-100 bg-gray-50"}`}>
        <div>
          <p className="text-[14px] font-semibold text-gray-900 mb-0.5">Enable Reminder</p>
          <p className="text-[12px] text-gray-500">Send an automated reminder before the deadline</p>
        </div>
      ...
      {reminder.enabled && (
        <div className="space-y-4">
          <div>
            <p className="text-[12px] font-semibold text-gray-700 mb-3 uppercase tracking-wide">Send reminder</p>
            <div className="flex gap-2 flex-wrap">
              {["24 hours before", "48 hours before", "3 days before"].map((t) => (
                <button
                  key={t}
                  onClick={() => setReminder({ ...reminder, timing: t })}
                  className={`px-4 py-2 rounded-full text-[12px] font-medium border transition-all ${
                    reminder.timing === t
          ...
          <div>
            <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Reminder Email Body</label>
            <Textarea
              value={reminder.emailContent}
              onChange={(e) => setReminder({ ...reminder, emailContent: e.target.value })}
              className="w-full min-h-[200px] border border-gray-200 rounded-xl px-3 py-2.5 text-[13px] text-gray-800 font-mono leading-relaxed focus:outline-none focus:ring-1 focus:ring-amber-400 resize-none bg-white"
            />
          </div>
        </div>
      )}
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h2 className="text-[18px] font-bold text-gray-900 mb-0.5">Filtering & Approval Rules</h2>
          <p className="text-[12px] text-gray-500">Define how the AI handles assessment results</p>
        </div>
      </div>

      {/* Score threshold */}
      <div className="bg-white border border-gray-100 rounded-2xl p-5 mb-4 shadow-sm">
        <p className="text-[13px] font-semibold text-gray-900 mb-1">Auto-rejection threshold</p>
        <p className="text-[12px] text-gray-400 mb-4">Candidates scoring below this will be flagged for rejection</p>
        <div className="flex items-center gap-4">
          <div className="flex-1">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[12px] text-gray-500">0</span>
              <span className="text-[14px] font-bold text-indigo-700">{criteria.threshold} / 5</span>
              <span className="text-[12px] text-gray-500">5</span>
            </div>
            <input
            ...
            <div className="flex justify-between mt-1.5">
              {[1, 2, 3, 4, 5].map((v) => (
                <span key={v} className={`text-[10px] font-medium ${criteria.threshold >= v ? "text-indigo-600" : "text-gray-300"}`}>{v}</span>
              ))}
            </div>
          </div>
        </div>
        <div className="mt-4 bg-red-50 border border-red-100 rounded-xl px-4 py-3 flex items-center gap-2">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-5 mb-4 shadow-sm">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[13px] font-semibold text-gray-900">AI Auto-decision</p>
          </div>
          <Switch
            checked={criteria.autoDecide}
            onCheckedChange={(v) => setCriteria({ ...criteria, autoDecide: v })}
          />
        </div>
      ...
      {/* Human review gate */}
      <div className={`bg-white border-2 rounded-2xl p-5 shadow-sm transition-all ${criteria.humanReview ? "border-emerald-200" : "border-gray-100"}`}>
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            <Eye className="w-4 h-4 text-emerald-600" />
            <p className="text-[13px] font-semibold text-gray-900">Human review before rejection</p>
          </div>
          <Switch
            checked={criteria.humanReview}
            onCheckedChange={(v) => setCriteria({ ...criteria, humanReview: v })}
          />
        </div>
        <p className="text-[12px] text-gray-400 leading-relaxed">
      ...
      <div className="mt-4 bg-indigo-50 border border-indigo-100 rounded-2xl px-5 py-4">
        <p className="text-[12px] font-semibold text-indigo-700 mb-2">Summary</p>
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-[52rem] mx-4 flex flex-col overflow-hidden max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-8 py-5 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-[15px] font-bold text-gray-900">Assessment Agent Setup</h1>
        ...
        <div className="px-8 py-4 border-t border-gray-100 flex items-center justify-between bg-gray-50/50">
          ...
          <div className="flex items-center gap-2">
            {STEPS.map((s) => (
              <div key={s.id} className={`rounded-full transition-all ${step === s.id ? "w-4 h-2 bg-indigo-600" : "w-2 h-2 bg-gray-300"}`} />
            ))}
          </div>
          ...
          <Button
            onClick={handleNext}
            disabled={generating}
            className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 h-9 rounded-xl text-[13px] font-semibold flex items-center gap-1.5"
          >
            {generating ? (
